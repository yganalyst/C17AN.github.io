...
아무래도 자바스크립트를 주로 사용하다 보니 저절로 node.js에 관심이 생기게 되었는데, 많은 사람들이 `socket.io` 를 사용하는 것 같아 알아보게 되었다.
...
그런데, 코드를 들여다보면 .on 과 .emit 메서드가 `io.on` 과 `socket.on`, `io.emit` 과 `socket.emit` 으로 또다시 갈라짐을 알 수 있었다.
