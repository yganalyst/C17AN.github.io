...
이게 약간 번거롭다고 느껴 좋은 방법이 없을까 찾아봤는데, 역시 나처럼 생각하는 사람이 또 있었나보다. 🤣
...
[**Live Server**](https://marketplace.visualstudio.com/items?itemName=ritwickdey.LiveServer) 는 VS Code에서 제공되는 익스텐션(확장 프로그램)으로, HTML과 CSS 의 수정사항을 새로고침 없이 실시간으로 확인할 수 있는 강력한 도구다.
...
물론 출력만 하면 이걸 설치한 보람이 없을 테니, 한번 HTML 문서를 내 입맛대로 바꿔 보자.

그러면 더이상 번거로운 새로고침 없이 자동으로 HTML 문서와 CSS 의 변경사항이 적용된 화면이 나타나는 모습을 볼 수 있을 것이다. 😄
...
나처럼 불필요한 일을 불필요하게 생각하는 사람이 만든 멋진 도구가 이미 올라와 있을 수도 있을 테니 말이다! 😆
